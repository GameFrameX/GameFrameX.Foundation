...
[![Documentation](https://img.shields.io/badge/docs-gameframex-brightgreen.svg)](https://gameframex.doc.alianblank.com/ja/server/foundation)
...
[ドキュメント](https://gameframex.doc.alianblank.com/ja/server/foundation) · [クイックスタート](https://gameframex.doc.alianblank.com/ja/server/foundation) · QQグループ: 467608841 / 233840761
...
GameFrameX.Foundation は、GameFrameX サーバープロジェクト向けに横断的な基盤機能を提供する .NET 10 クラスライブラリのモジュール群です。ローカライゼーション、JSON、オプションパース、ORM 属性、拡張メソッド、暗号化、ハッシュ、ユーティリティ、ロギング、HTTP ヘルパー、レスポンス正規化、ORM 基底エンティティ、冪等性などをカバーします。
...
設計原則と依存関係グラフについては、[アーキテクチャドキュメント](https://gameframex.doc.alianblank.com/ja/server/foundation)をご参照ください。
...
| ORM Attribute      | `GameFrameX.Foundation.Orm.Attribute`      | ORM attribute definitions for audit, cache, soft-delete, tenant, data scope, sensitive masking, tree, import/export | Zero dependencies   | [Docs](https://gameframex.doc.alianblank.com/ja/server/foundation/orm-attribute)      |
...
| Idempotency        | `GameFrameX.Foundation.Idempotency`        | Idempotency occupation/replay decisions and universal event envelope | → Utility           | [Docs](https://gameframex.doc.alianblank.com/ja/server/foundation/idempotency)        |
...
GameFrameX.Foundation のバージョン履歴については、[CHANGELOG.md](CHANGELOG.md) をご参照ください。

## ライセンス

詳しくは [LICENSE.md](LICENSE) をご参照ください。
